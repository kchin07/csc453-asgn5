...
void print_ptable(FILE* dest, struct partition ptable[], struct options* opts){
   int i;
...
struct partition* read_ptable(struct disk* disk, struct options* opts,
...
struct disk* open_disk(char* fileName, char* mode, struct options* opts,
   struct disk* disk){
   ...
   if(opts->part != INACTIVE){
      if(!read_ptable(disk, opts, ptable)){
         close_disk(disk);
         return NULL;
      }
      if(verbose_mode){
      ...
      if(ptable[opts->part].type == TYPEMINIX){
         disk->base = ptable[opts->part].lFirst * SECTIONSIZE;
         disk->size = ptable[opts->part].size * SECTIONSIZE;
      }
      else{
      ...
      if(opts->subpart != INACTIVE){
         ...
         if(ptable[opts->subpart].type == TYPEMINIX){
            disk->base = ptable[opts->subpart].lFirst * SECTIONSIZE;
            disk->size = ptable[opts->subpart].size * SECTIONSIZE;
         }
         else{
...
int main(){
   ...
   char* headers = "boot start_head start_sec start_cyl type last_head last_sec"
                   " last_cyl log_first size\n";
   printf(headers);
   int mode = 0x100;
   printf("%d\n", mode);
   return 0;
